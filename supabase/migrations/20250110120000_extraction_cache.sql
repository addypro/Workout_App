...
DROP POLICY IF EXISTS "Cache is readable by all" ON public.extraction_cache;
DROP POLICY IF EXISTS "Only service role can insert cache" ON public.extraction_cache;
DROP POLICY IF EXISTS "Only service role can update cache" ON public.extraction_cache;
